  ...
  function ChatContainer() {
    ...
    return (
      <div
        ref={containerRef}
        className="ChatContainer relative w-full max-w-3xl h-[70vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600
        flex flex-col gap-4 p-4 rounded-2xl bg-white dark:bg-[#1B263B] shadow-2xl border border-gray-200 dark:border-[#324A5F] 
        animate-fade-in transition-colors duration-300"
      >
        {val.chat?.map((value, index) => (
          <div
            key={index}
            className={`group relative px-5 py-3 rounded-2xl text-sm max-w-[75%] w-fit break-words 
